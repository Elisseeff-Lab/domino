...
cor_scatter <- function(dom, tf, rec, remove_rec_dropout = TRUE, ...) {
    ...
    ggscatter(dat,
        x = "rec", y = "tf", add = "reg.line", conf.int = FALSE, cor.coef = FALSE,
...
plot_differential_linkages <- function(
    ...
    plot <- ComplexHeatmap::Heatmap(
        matrix = mat, cluster_rows = FALSE,
        left_annotation = ha_name, cell_fun = function(j, i, x, y, width, height, fill) {
        ...
        ), height = nrow(mat) * grid::unit(0.25, "in"), width = grid::unit(1, "in")
    ) + ha_subject
