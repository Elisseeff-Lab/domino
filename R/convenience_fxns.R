...
#' @param warning Logical indicating whether to print a warning if a cluster is not found in the domino object.
#' @return A domino object with clusters renamed in all applicable slots.
#' @keywords internal
#' @export 
#' 
#' @examples
#' dom <- rename_clusters(dominoSignal:::pbmc_dom_built_tiny, c("CD8_T_cell" = "CD8 T Cells", "B_cell" = "B Cells"))
rename_clusters <- function(dom, clust_conv, warning = FALSE) {
  ...
  if (dom@misc$create) {
    ...
    colnames(dom@misc$cl_rec_percent) <- plyr::revalue(colnames(dom@misc$cl_rec_percent), clust_conv, warn_missing = warning)
  }
  if (dom@misc$build) {
    ...
    names(dom@linkages$clust_incoming_lig) <- plyr::revalue(names(dom@linkages$clust_incoming_lig), clust_conv, warn_missing = warning)
    names(dom@linkages$clust_tf_rec) <- plyr::revalue(names(dom@linkages$clust_tf_rec), clust_conv, warn_missing = warning)
    ...
    names(dom@cl_signaling_matrices) <- plyr::revalue(names(dom@cl_signaling_matrices), clust_conv, warn_missing = warning)
